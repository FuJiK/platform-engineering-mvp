...
command -v clojure >/dev/null || { echo "Clojure CLI required"; exit 1; }
...
ok "clojure $(clojure --version 2>&1)"
...
| PLATFORM_ROLE=operator clojure -M -m platform-mvp.mcp 2>/dev/null \
...
| PLATFORM_ROLE=sre clojure -M -m platform-mvp.mcp 2>/dev/null \
